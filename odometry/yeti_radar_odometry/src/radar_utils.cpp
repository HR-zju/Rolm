...
void load_radar(std::string path, std::vector<int64_t> &timestamps, std::vector<double> &azimuths,
    ...
    cv::Mat raw_example_data_org = cv::imread(path, cv::IMREAD_GRAYSCALE);
    if (!raw_example_data_org.data) std::cout << "what fuck" << std::endl;
    cv::Mat raw_example_data;
    cv::rotate(raw_example_data_org, raw_example_data, cv::ROTATE_90_COUNTERCLOCKWISE);
    ...
    if (navtech_version == CIR204)
        range_bins = 3360;
//    range_bins = 576;
//    range_bins = raw_example_data.cols;
...
//            fft_data.at<float>(i, j) = ((float)*(byteArray  + j)) / 255.0;
//            if ((float)*(byteArray  + j) < 35) fft_data.at<float>(i, j) =0.0;
//            if (fft_data.at<float>(i, j)<0) fft_data.at<float>(i, j)=0.0;
